...
const Form = () => {
  ...
  const sendSignInData = async (event) => {
    event.preventDefault();
    setIsLoading(true);
    try {
  ...
  const togglePasswordPolicy = (event) => {
    event.preventDefault();
    setShowPasswordPolicy(!showPasswordPolicy);
  };
      ...
      <div className="form-cont" style={cont_style}>
        {isLoading ? (
          ...
          <form name="signup" className="signup-form">
            ...
            <div className="img-cont">{file && <img src={URL.createObjectURL(file)} alt="uploaded-img" />}</div>
            ...
            <button className="password-policy-btn" onClick={(event) => togglePasswordPolicy(event)}>
